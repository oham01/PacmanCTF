...
def create_team(first_index, second_index, is_red,
    first='OffensiveAgent', second='AStarDefensiveAgent', num_training=0):
...
class OffensiveAgent(CaptureAgent):
    ...
    def choose_action(self, game_state):
        ...
        """
        # Get visible enemies agent indices
        ...
        # If no enemies are visible, use simple evaluation
    ...
    def alpha_beta_expectimax(self, game_state, agent_index, depth, alpha, beta, enemies):
        ...
        """
        # Terminal state returns evaluation of the state
        if depth == 0:
        ...
        if next_agent in team_indices and next_agent != self.index:
        ...
        if agent_index == self.index:
            ...
            for action in legal_actions:
                ...
                _, future_value = self.alpha_beta_expectimax(next_state, next_agent, depth - 1, alpha, beta, enemies)
                score = self.evaluate(game_state, action) + future_value
        ...
        # Minimizing player (enemies)
            ...
            # Get ghost position and legal moves
            ...
            expected_score = 0
            probability = 1.0 / len(legal_ghost_actions) # Uniform distribution over legal moves

            # Iterate over all legal ghost actions, finding the one with the lowest score
...
class AStarDefensiveAgent(CaptureAgent):
    ...
    def register_initial_state(self, game_state):
        CaptureAgent.register_initial_state(self, game_state)
        self.patrol_index = 0
        self.border_points = self.get_border_points(game_state)
    ...
    def get_weights(self, game_state, action):
        ...
        if len(invaders) > 0:
            # When there are invaders, prioritize interception
    ...
    def get_border_points(self, game_state):
        """
        Get list of border positions to patrol
        """
        ...
        if food:
            border_points.sort(key=lambda x: min([self.get_maze_distance(x, food_pos) for food_pos in food]))
    ...
    def patrol_border(self, game_state):
        ...
        # If we've reached our current patrol point
        current_target = self.border_points[self.patrol_index]
        if (self.get_maze_distance(my_pos, current_target) <= 1):
